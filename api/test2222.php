        ...
        if(is_uploaded_file($_FILES['file']['tmp_name'])){
           ...
           // $errorEmail=array();
            ...
            while(($line = fgetcsv($csvFile)) !== FALSE){
                ...
                $leaveStatus                 = $line[71]; 
                $leaveType                   = $line[72]; 
                $leaveEffectDate             = $line[73]; 
                $leaveResumeDate             = $line[74]; 
                $leaveNoDays                 = $line[75]; 
                $leaveExpireDate             = $line[76]; 
                $groupCadre                  = $line[77]; 
                $staffType                   = $line[78]; 
                $initial                     = $line[79]; 
                $CONTSRTDATE                 = $line[80]; 
                $CONTEXPDATE                 = $line[81]; 
                $NoOfRenewedContract         = $line[82]; 
                //bursary
                $bursaryNo                   = $line[83]; 
                $IPPIS_NO                    = $line[84]; 
                $SalaryCategory              = $line[85]; 
                $bankName                    = $line[86]; 
                $accountNumber               = $line[87]; 
                $PFANAme                     = $line[88]; 
                $PENSION_PIN                 = $line[89];
                //$lastPromDate                = $line[90]; 

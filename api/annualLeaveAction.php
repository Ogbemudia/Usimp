...
if($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['crud_req'] == "leaveAction" && $_POST['uniqueId']){
    ...
    $action = $_POST['action'];
    /* **************************************************
    update leave profile for defermen of Annual leave
    *****************************************************/
    if($action==="defer"){
        $data=array(
            'registry.accumulativeLeave.leaveNoDays'=>$accuLeaveDuration1,
            //'registry.accumulativeLeave.'.$datehist=>[
                //$datehist=>[
                   
                        'registry.accumulativeLeave.accumulatedLeaves.' . $datehist => [
                            'leaveName'     => $leaveName,
                            'leaveDuration' =>(int) $leaveDuration,
                            'deferReason'  =>$_POST['comments'],
                            'deferDate'     =>$created,
        ...
        *****************************************************/
        ...
        if ($histryUpdated) {
            ...
            $data=array(
                'registry.annualLeave.status'     => 'defered',
                /*'registry.annualLeave.name'       => $leaveName,  
            ...
            array('success' => 0,
            'status' => 400,
            'message' => 'Leaves deferment failed try again later. Accumulative leave not updated'));
            exit;
        };
    ...
    *****************************************************/
        ...
        *****************************************************/
        ...
        if ($histryUpdated) {
            $data=array(
                'registry.annualLeave.status'     => 'forfeited',  
                /* 'registry.annualLeave.name'       => '',  
                'registry.annualLeave.startDate'  => '',  
                'registry.annualLeave.endDate'     => '',  
                'registry.annualLeave.leaveNoDays' => (int) 0,  
